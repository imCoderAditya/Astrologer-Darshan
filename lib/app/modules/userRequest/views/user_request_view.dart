...
class UserRequestView extends GetView<UserRequestController> {
  ...
  const UserRequestView({super.key, this.sessionType});
    ...
    return Obx(() {
      ...
      return RefreshIndicator(
        onRefresh: () async {
          ...
          controller.fetchUserRequest(sessionType: sessionType);
        },
  ...
  Widget _buildSessionHeader(Session? session) {
    return Row(
      children: [
        _buildAstrologerAvatar(session?.customer?.profilePicture),
        SizedBox(width: 12.w),
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
